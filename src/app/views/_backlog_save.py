...
from app.views._backlog_helpers import check_rewatch_cancelled, update_pin_order
...
def commit_backlog_form(request, media, media_type, form, pre_save):
    ...
    update_pin_order(request.user, media, old_is_pinned, is_pinned_submitted)
    logger.info("%s updated from backlog.", form.instance)
    rewatch_cancelled = check_rewatch_cancelled(
        request.user,
        media,
        media_type,
        old_is_rewatch,
