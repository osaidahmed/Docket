...
from users.models import HomeSortChoices, HomeTruncationChoices
...
class LayoutGroupToggleTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        # Anime with upcoming event (has unit: Episode)
        cls.anime_item = Item.objects.create(
            media_id="1",
            source=Sources.MAL.value,
            media_type=MediaTypes.ANIME.value,
        ...
        # TV with upcoming event (NO unit — previously crashed with KeyError)
        ...
        # Movie (NO unit, planning status — different group)
        movie_item = Item.objects.create(
            media_id="238",
            source=Sources.TMDB.value,
            media_type=MediaTypes.MOVIE.value,
    ...
    def test_table_hides_type_column_when_grouped_by_type(self):
        ...
        # The type icon cell should also be hidden
    ...
    def test_truncation_skipped_for_status_grouping(self):
        ...
        response = self.client.get(reverse("home") + "?layout=cards&group=status")
        self.assertEqual(response.status_code, 200)
        # All items should be visible — truncation only applies to type grouping
    ...
    def test_truncation_works_for_type_grouping(self, mock_metadata):
        ...
        self.user.home_truncation = HomeTruncationChoices.THREE
        self.user.save(update_fields=["home_truncation"])
        # We have anime(1)+tv(1)+game(1) in progress, movie(1) planning.
        # Need >3 items in one type/status to trigger truncation.
        # Create 3 more anime items to get 4 total (exceeds limit of 3).
        for i in range(3):
            item = Item.objects.create(
...
class SeasonCollapsingTests(TestCase):
    ...
    def _create_season(self, media_id, source, season_number, title, **kwargs):
        item = Item.objects.create(
            media_id=media_id,
            source=source,
            media_type=MediaTypes.SEASON.value,
    ...
    def test_multiple_seasons_collapsed(self):
        ...
        self._create_season("show-1", Sources.TMDB.value, 1, "Show X")
        self._create_season("show-1", Sources.TMDB.value, 2, "Show X")
        self._create_season("show-1", Sources.TMDB.value, 3, "Show X")
    ...
    def test_single_season_not_collapsed(self):
        ...
        self._create_season("show-2", Sources.TMDB.value, 1, "Show Y")
    ...
    def test_pinned_season_not_collapsed(self):
        ...
        self._create_season("show-3", Sources.TMDB.value, 1, "Show Z")
        self._create_season("show-3", Sources.TMDB.value, 2, "Show Z")
        self._create_season("show-3", Sources.TMDB.value, 3, "Show Z", pin_order=0)
    ...
    def test_collapse_only_in_planning(self):
        ...
        s1 = self._create_season("show-4", Sources.TMDB.value, 1, "Show W")
        s1.status = Status.IN_PROGRESS.value
        s1.save()
        s2 = self._create_season("show-4", Sources.TMDB.value, 2, "Show W")
    ...
    def test_collapse_badge_in_template(self):
        ...
        self._create_season("show-5", Sources.TMDB.value, 1, "Show V")
        self._create_season("show-5", Sources.TMDB.value, 2, "Show V")
        self._create_season("show-5", Sources.TMDB.value, 3, "Show V")
...
class MediaListPinnedTests(TestCase):
    ...
    def test_pinned_list_empty_for_non_planning(self):
        """pinned_list is empty when status filter is not Planning."""
        Anime.objects.create(
            item=self.anime_item_a,
            user=self.user,
            status=Status.IN_PROGRESS.value,
        ...
        self.assertEqual(response.context["pinned_list"], [])
    ...
    def test_pin_toggle_hidden_for_non_planning(self):
        """Pin toggle button does not appear on non-Planning cards."""
        Anime.objects.create(
            item=self.anime_item_a,
            user=self.user,
            status=Status.IN_PROGRESS.value,
