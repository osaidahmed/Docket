...
class MediaDetailsViewTests(TestCase):
    ...
    @patch("app.providers.services.get_media_metadata")
    def test_media_details_with_related(self, mock_get_metadata):
        mock_get_metadata.return_value = {
            "media_id": "238",
            "title": "Test Movie",
            "media_type": MediaTypes.MOVIE.value,
        ...
        self.assertEqual(response.status_code, 200)
        related = response.context["media"]["related"]
        self.assertIn("recommendations", related)
        self.assertEqual(len(related["recommendations"]), 1)
        self.assertEqual(related["similar"], [])

    @patch("app.providers.services.get_media_metadata")
    @patch("app.providers.tmdb.process_episodes")
    def test_season_details_with_related(self, mock_process, mock_get_metadata):
        mock_get_metadata.return_value = {
            "title": "Test TV Show",
            "media_id": "1668",
            "source": Sources.TMDB.value,
        ...
        self.assertEqual(response.status_code, 200)
        related = response.context["media"]["related"]
        self.assertIn("seasons", related)

    @patch("app.providers.services.get_media_metadata")
    @patch("app.providers.manual.process_episodes")
    def test_season_details_manual_source(self, mock_process, mock_get_metadata):
        mock_get_metadata.return_value = {
            "title": "Manual TV",
            "media_id": "1",
