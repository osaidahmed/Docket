...
class SeasonAutoFillTests(TestCase):
    ...
    def _mock_metadata(self, media_type, _media_id, _source, *args):
        """Side-effect returning a 5-season show with 2 episodes per season."""
    ...
    def test_completing_season3_backfills_prior_seasons(self, mock_get_metadata):
        """Completing season 3 creates seasons 1-2 as COMPLETED with episodes."""
    ...
    def test_starting_season3_backfills_prior_seasons(self, mock_get_metadata):
        """Starting season 3 (IN_PROGRESS) backfills seasons 1-2 as COMPLETED."""
        mock_get_metadata.side_effect = self._mock_metadata
    ...
    def test_completing_season1_does_not_backfill(self, mock_get_metadata):
        """Completing season 1 does NOT trigger backfill (no prior seasons)."""
        mock_get_metadata.side_effect = self._mock_metadata
        ...
        # No new seasons created by backfill (season 1 has no prior)
        self.assertEqual(
    ...
    def test_existing_completed_season_not_duplicated(self, mock_get_metadata):
        """Existing COMPLETED seasons are skipped during backfill."""
        mock_get_metadata.side_effect = self._mock_metadata
    ...
    def test_existing_non_completed_season_force_completed(self, mock_get_metadata):
        """Existing non-COMPLETED prior seasons are force-completed on backfill."""
        mock_get_metadata.side_effect = self._mock_metadata
    ...
    def test_paused_does_not_trigger_autofill(self, mock_get_metadata):
        """Setting status to PAUSED does NOT trigger auto-fill."""
        mock_get_metadata.side_effect = self._mock_metadata
    ...
    def test_dropped_does_not_trigger_backfill(self, mock_get_metadata):
        """Setting status to DROPPED does NOT trigger backfill."""
        mock_get_metadata.side_effect = self._mock_metadata
    ...
    def test_planning_forward_fills_subsequent_seasons(self, mock_get_metadata):
        """Changing season 1 to PLANNING creates subsequent seasons as PLANNING."""
        mock_get_metadata.side_effect = self._mock_metadata
        ...
        # Simulate form save: default status (COMPLETED) then changed to PLANNING
        season1 = Season(
            item=season1_item,
            user=self.user,
            related_tv=self.tv,
        ...
        # Seasons 2, 4, 5 created as PLANNING (3 already exists)
        for sn in [2, 4, 5]:
            season = Season.objects.get(
                item__media_id="456",
                item__season_number=sn,
        ...
        # No episodes on forward-filled seasons
        for sn in [2, 4, 5]:
            season = Season.objects.get(
                item__media_id="456",
                item__season_number=sn,
    ...
    def test_forward_fill_does_not_duplicate_existing(self, mock_get_metadata):
        """Forward-fill skips seasons that already exist."""
        mock_get_metadata.side_effect = self._mock_metadata
        ...
        # Season 3 (from setUpTestData) should NOT be duplicated
        self.assertEqual(
            Season.objects.filter(
                item__media_id="456",
    ...
    def test_integration_backfill_and_auto_advance(self, mock_get_metadata):
        """Complete S3, backfill 1-2, create eps for 3, advance to 4."""
        mock_get_metadata.side_effect = self._mock_metadata
        ...
        # Seasons 1-2 backfilled as COMPLETED with episodes
        for sn in [1, 2]:
            season = Season.objects.get(
                item__media_id="456",
                item__season_number=sn,
        ...
        # Season 3 has episodes
        self.assertEqual(self.season3.episodes.count(), 2)

        # Season 4 auto-advanced to IN_PROGRESS
