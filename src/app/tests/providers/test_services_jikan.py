...
class JikanBrowseScheduleTests(TestCase):
    """Test Jikan browse_schedule functionality."""

    def setUp(self):
        cache.clear()

    @patch("app.providers.services.api_request")
    def test_schedule_returns_list(self, mock_api):
        mock_api.return_value = {
            "data": [
                {
                    "mal_id": 1,
                    ...
                    "broadcast": {"time": "18:00", "day": "Monday"},
                }
            ]
        }
        result = jikan.browse_schedule()
        self.assertIsInstance(result, list)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["media_id"], 1)
        self.assertEqual(result[0]["title"], "Test Anime")
        self.assertEqual(result[0]["broadcast_time"], "18:00")
        self.assertEqual(result[0]["broadcast_day"], "Monday")
        self.assertEqual(result[0]["source"], Sources.MAL.value)

    @patch("app.providers.services.api_request")
    def test_schedule_with_day_filter(self, mock_api):
        mock_api.return_value = {"data": []}
        jikan.browse_schedule(day="Monday")
        _, kwargs = mock_api.call_args
        self.assertEqual(kwargs["params"]["filter"], "monday")

    @patch("app.providers.services.api_request")
    def test_schedule_no_broadcast_info(self, mock_api):
        mock_api.return_value = {
            "data": [
                {
                    "mal_id": 2,
        ...
        result = jikan.browse_schedule()
        self.assertEqual(result[0]["broadcast_time"], "")
        self.assertEqual(result[0]["broadcast_day"], "")

    @patch("app.providers.services.api_request")
    def test_schedule_error_returns_empty(self, mock_api):
        mock_api.side_effect = Exception("API error")
        result = jikan.browse_schedule()
        self.assertEqual(result, [])

    @patch("app.providers.services.api_request")
    def test_schedule_cached(self, mock_api):
        mock_api.return_value = {"data": []}
        jikan.browse_schedule()
        jikan.browse_schedule()
